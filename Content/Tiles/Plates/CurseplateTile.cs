...
{
    public class CurseplateTile : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            if (!Main.dedServ)
            {
                PulseTexture = Request<Texture2D>("CalRemix/Content/Tiles/Plates/CurseplateTilePulse", (AssetRequestMode)1).Value;
                PulseColors = (Color[])(object)new Color[PulseTexture.Width];
                Main.QueueMainThreadAction(delegate
                {
                    PulseTexture.GetData(PulseColors);
                });
                GlowTexture = Request<Texture2D>("CalRemix/Content/Tiles/Plates/CurseplateTileGlow", (AssetRequestMode)1).Value;
            }
            ...
            AddMapEntry(new Color(29, 124, 143), null);
        }
