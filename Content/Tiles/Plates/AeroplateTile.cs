...
namespace CalRemix.Content.Tiles.Plates
{
    public class AeroplateTile : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            if (!Main.dedServ)
            {
                PulseTexture = Request<Texture2D>("CalRemix/Content/Tiles/Plates/AeroplateTilePulse", (AssetRequestMode)1).Value;
                PulseColors = (Color[])(object)new Color[PulseTexture.Width];
                Main.QueueMainThreadAction(delegate
                {
                    PulseTexture.GetData<Color>(PulseColors);
                });
                GlowTexture = Request<Texture2D>("CalRemix/Content/Tiles/Plates/AeroplateTileGlow", (AssetRequestMode)1).Value;
            }
