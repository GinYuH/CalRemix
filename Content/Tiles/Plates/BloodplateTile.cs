...
{
    public class BloodplateTile : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            if (!Main.dedServ)
            {
                PulseTexture = Request<Texture2D>("CalRemix/Content/Tiles/Plates/BloodplateTilePulse", (AssetRequestMode)1).Value;
                PulseColors = (Color[])(object)new Color[PulseTexture.Width];
                Main.QueueMainThreadAction(delegate
                {
                    PulseTexture.GetData<Color>(PulseColors);
                });
                GlowTexture = Request<Texture2D>("CalRemix/Content/Tiles/Plates/BloodplateTileGlow", (AssetRequestMode)1).Value;
            }
            ...
            MineResist = 1f;
            DustType = DustID.Rain_BloodMoon;
