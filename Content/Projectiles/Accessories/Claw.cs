    ...
    public class Claw : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                if (Projectile.Center.Distance(victim.Center) > 60 && victim.GetGlobalNPC<CalRemixNPC>().clawed <= 0)
                {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D ClawBottom = ModContent.Request<Texture2D>("CalRemix/Content/Projectiles/Accessories/ClawBottom").Value;
        ...
        public override void OnHitNPC(NPC targete, NPC.HitInfo hit, int damageDone)
        {
            if (targete.active && targete.life > 0 && targete.whoAmI == target && !targete.dontTakeDamage)
            {
                ...
                {
                    targete.GetGlobalNPC<CalRemixNPC>().clawPosition = Main.player[Projectile.owner].GetModPlayer<CalRemixPlayer>().clawPosition;
                }
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (latched.active && latched != null && latched.GetGlobalNPC<CalRemixNPC>().clawed > 0)
            {
                latched.GetGlobalNPC<CalRemixNPC>().clawed = 0;
            }
            ...
            if (Main.netMode != NetmodeID.Server)
            {
                for (int i = 0; i < 4; i++)
                {
                    Gore.NewGore(Projectile.GetSource_Death(), Projectile.position, Projectile.velocity, Mod.Find<ModGore>("Claw" + (i + 1)).Type, 1f);
                }
            }
