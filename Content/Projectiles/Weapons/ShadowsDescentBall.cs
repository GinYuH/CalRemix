...
namespace CalRemix.Content.Projectiles.Weapons
{
    public class ShadowsDescentBall : ModProjectile
	{
        ...
        {
            modifiers.SourceDamage *= (int)target.GetGlobalNPC<CalRemixNPC>().shadowHit;
        }
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            target.AddBuff(BuffID.ShadowFlame, 600);
            if (target.GetGlobalNPC<CalRemixNPC>().shadowHit < 5)
            {
                target.GetGlobalNPC<CalRemixNPC>().shadowHit += 0.2f;
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texture = (Texture2D)ModContent.Request<Texture2D>("CalRemix/Content/Projectiles/Weapons/ShadowsDescentBall");
            ...
            if (Projectile.velocity == Vector2.Zero)
            {
                Texture2D texture2 = (Texture2D)ModContent.Request<Texture2D>("CalRemix/Content/Projectiles/Weapons/Shadow");
