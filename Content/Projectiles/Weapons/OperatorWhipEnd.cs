...
{
    public class OperatorWhipEnd : ModProjectile
    {
        ...
        public static void FillWhipControlPoints(Projectile proj, List<Vector2> controlPoints)
        {
            ...
            for (int i = 0; i < segments; i++)
            {
                ...
                num2 += num6 * proj.localAI[1];
                num4 += num6 * proj.localAI[1];
                num3 += num6 * proj.localAI[1];
                vector = vector4;
                vector3 = vector5;
                vector2 = vector7;
            }
        ...
        {
            target.AddBuff(BuffID.ScytheWhipEnemyDebuff, 240);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < list.Count - 1; i++)
            {
                ...
                {
                    texture = ModContent.Request<Texture2D>("CalRemix/Content/Projectiles/Weapons/OperatorWhipSegment").Value;
                }
                ...
                Main.EntitySpriteDraw(texture, pos - Main.screenPosition, null, color, rotation + MathHelper.PiOver2, texture.Size() / 2, scale, flip, 0);
