...
{
    public class PizzaWheelProj : ModProjectile
    {
        ...
        public override void AI()
        {
            if (Projectile.ai[2] == 1)
            {
                ...
                Main.LocalPlayer.Calamity().GeneralScreenShakePower = MathHelper.Max(Main.LocalPlayer.Calamity().GeneralScreenShakePower, 1f);
            }
            else
            {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Projectile.ai[2] == 0)
            {
                for (int i = 0; i < Main.rand.Next(2, 4); i++)
                {
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        spawnPos.Y += Main.rand.Next(-20, 20);
                        int p = Projectile.NewProjectile(Projectile.GetSource_OnHit(target), spawnPos, spawnPos.DirectionTo(target.Center).X * Vector2.UnitX * Main.rand.NextFloat(6, 9), Type, Projectile.damage, Projectile.knockBack, Projectile.owner, ai2: 1);
            ...
            {
                SoundEngine.PlaySound(new SoundStyle("CalRemix/Assets/Sounds/PizzaHit") with { MaxInstances = 0 }, target.Center);
                for (int i = 0; i < 10; i++)
                {
                    GeneralParticleHandler.SpawnParticle(new SquareParticle(target.Center, Main.rand.NextVector2Circular(10, 10), false, 10, Main.rand.NextFloat(1f, 2f), Color.Yellow));
                }
            }
        }
    }
