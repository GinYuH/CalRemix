...
{
    public class CombosamaSpin : ModProjectile
    {
        public VertexStrip TrailDrawer;
        Player Owner => Main.player[Projectile.owner];
        public override string Texture => "CalRemix/Content/Items/Weapons/Combosama";
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] > 60 && Projectile.ai[0] % 30 == 0)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Owner.DirectionTo(Main.MouseWorld) * 20, ModContent.ProjectileType<NamelessVortex>(), Projectile.damage, Projectile.knockBack, Owner.whoAmI, ai0: 1, ai2: Main.rand.Next(0, 3));
            }
            StickToOwner();
            AdjustPlayerValues();
            if (!Owner.controlUseTile || Owner.HeldItem.type != ModContent.ItemType<Combosama>() || Owner.CCed)
            {
                Projectile.Kill();
            }
            if (Owner.controlUp)
            {
                Owner.Remix().muraregen = true;
                Vector2 spawnPos = Main.rand.NextVector2FromRectangle(Projectile.Hitbox);
                GeneralParticleHandler.SpawnParticle(new SquareParticle(spawnPos, spawnPos.DirectionTo(Projectile.Center) * 8, false, 60, Main.rand.NextFloat(2f, 4f), Color.Violet));
                GeneralParticleHandler.SpawnParticle(new SquareParticle(spawnPos, spawnPos.DirectionTo(Projectile.Center) * 8, false, 60, Main.rand.NextFloat(2f, 4f), Color.Yellow));
                GeneralParticleHandler.SpawnParticle(new SquareParticle(spawnPos, spawnPos.DirectionTo(Projectile.Center) * 8, false, 60, Main.rand.NextFloat(2f, 4f), Color.Red));
                if (Projectile.ai[0] % 90 == 0)
                {
                    GeneralParticleHandler.SpawnParticle(new PulseRing(Projectile.Center, Vector2.Zero, Color.Magenta, 0, 2, 60));
                }
            }
            else
            {
                Owner.Remix().muraregen = false;
            }
        }
        ...
        public Color TrailColorFunction(float completionRatio)
        {
            ...
            return CalamityUtils.MulticolorLerp(MathHelper.Clamp(completionRatio * 1.4f, 0f, 1f), Color.Violet, Color.Red, Color.PaleGoldenrod) * opacity;
        }
