...
{
    public class EldritchPulse : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft % 50 == 1 && Projectile.alpha <= 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    for (int i = 0; i < 16; i++)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, velocity, ModContent.ProjectileType<EldritchSpirit>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                    }
                    for (int i = 0; i < 16; i++)
                    {
            ...
            if (Projectile.timeLeft % 150 == 1 && target != null && target.active && Projectile.timeLeft > 60)
            {
                ...
                {
                    int p = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, direction, ModContent.ProjectileType<EldritchRay>(), (int)(Projectile.damage * 1.5f), 0f, Projectile.owner, angularChange, (float)Projectile.whoAmI);
