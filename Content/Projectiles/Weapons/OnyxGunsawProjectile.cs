...
{
    public class OnyxGunsawProjectile : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D glowmaskTexture = ModContent.Request<Texture2D>("CalRemix/Content/Projectiles/Weapons/OnyxGunsawGlowmask").Value;
        ...
        public override void AI()
        {
            ...
            if (Time % 4 == 0)
            {
                ...
                Projectile shard = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 30, ProjectileID.BlackBolt, Projectile.damage * 3, 0, Projectile.owner, 0f, 0f);
                shard.timeLeft = 600;
                for (int i = 0; i < 3; i++)
                {
                    float randAngle = Main.rand.NextFloat(0.035f);
                    float randVelMultiplier = Main.rand.NextFloat(0.92f, 1.08f);
                    Vector2 ccwVelocity = Projectile.velocity.RotatedBy(-randAngle) * randVelMultiplier;
                    Vector2 cwVelocity = Projectile.velocity.RotatedBy(randAngle) * randVelMultiplier;
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, ccwVelocity * 20, ProjectileID.Bullet, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, cwVelocity * 20, ProjectileID.Bullet, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                }
            }
        ...
        public void ReleasePrismTeeth()
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Owner.Center, Projectile.velocity, ModContent.ProjectileType<OnyxPrismTooth>(), (int)ToothDamage, 0f, Projectile.owner, shootReach, Projectile.whoAmI, Projectile.ai[2]);
        }
