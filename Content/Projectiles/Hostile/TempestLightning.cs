...
{
    public class TempestLightning : BaseLaserbeamProjectile
    {
        public override string Texture => "CalamityMod/Projectiles/InvisibleProj";
        public NPC Owner => Projectile.ai[1] > -1 ? Main.npc[(int)Projectile.ai[1]] : null;
        public override Color LightCastColor => Projectile.ai[2] > 60 ? Color.Cyan * 0.5f : Color.DarkCyan;
        public override float Lifetime => 18000f;
        public override float MaxScale => 1f;
        public override float MaxLaserLength => 3200f; //100 tiles
        public override Texture2D LaserBeginTexture => ModContent.Request<Texture2D>("CalamityMod/Projectiles/InvisibleProj").Value;
        public override Texture2D LaserMiddleTexture => ModContent.Request<Texture2D>("CalamityMod/Projectiles/InvisibleProj").Value;
        public override Texture2D LaserEndTexture => ModContent.Request<Texture2D>("CalamityMod/Projectiles/InvisibleProj").Value;
        ...
        public override bool PreAI()
        {
            if (Owner != null)
            {
                if (!Owner.active || Owner.life <= 0 || Owner.type != ModContent.NPCType<TempestKraken>())
                {
                    Projectile.Kill();
                    return false;
                }
            ...
            {
                SoundEngine.PlaySound(SoundID.DD2_LightningBugZap with { Volume = 0.4f }, Projectile.Center);
            }
            if (Projectile.ai[2] == 60)
            {
                SoundEngine.PlaySound(CalamityMod.NPCs.SupremeCalamitas.SupremeCalamitas.BrimstoneShotSound with { Volume = 0.4f }, Projectile.Center);
            }
        ...
        internal float BackgroundWidthFunction(float completionRatio, Vector2 v) => WidthFunction(completionRatio, v) * 10f;

        internal Color BackgroundColorFunction(float completionRatio, Vector2 v)
        {
            return Projectile.ai[2] > 60 ? Color.LightBlue * 0.6f : Color.DarkCyan * 0.4f;
        }
