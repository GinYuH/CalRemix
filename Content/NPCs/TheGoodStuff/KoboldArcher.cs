...
{
    public class KoboldArcher : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (Collision.CanHitLine(NPC.Center, 1, 1, Main.player[NPC.target].Center, 1, 1) && NPC.Distance(Main.player[NPC.target].Center) < 920)
            {
                ...
                if (NPC.Calamity().newAI[0] % 80 == 0)
                {
                    ...
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, NPC.DirectionTo(Main.player[NPC.target].Center) * 12, ProjectileID.WoodenArrowHostile, CalRemixHelper.ProjectileDamage(26, 58), 1f);
                    }
                }
            }
        }
        ...
        public override void HitEffect(NPC.HitInfo hit)
        {
            ...
            if (NPC.life <= 0)
            {
                ...
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Gore.NewGore(NPC.GetSource_Death(), NPC.position, NPC.velocity, Mod.Find<ModGore>("KoboldArcher1").Type, NPC.scale);
                }
            }
        }
    }
