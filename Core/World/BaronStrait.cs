...
namespace CalRemix.Core.World
{
    public class BaronStrait : ModSystem
    {
        public static void GenerateBaronStrait(bool? left, bool full = false)
        {
            ...
            // Stretch the top corner of the map
            Cornerlands(left);

            // On the seed, leave a small pocket where the player can safely spawn
        ...
        public static void GeneratePocket()
        {
            // Generate a small rectangle and set the player's spawn point in it
            int centerX = Main.maxTilesX / 2;
            int centerY = Main.maxTilesY / 4;
            int radius = 22;
            for (int i = centerX - radius; i < centerX + radius; i++)
            {
                for (int j = centerY - radius; j < centerY + 5; j++)
                {
            ...
            for (int gdv = 0; gdv < 1; gdv++)
            {
                for (int i = 0; i < straitRect.Width; i++)
                {
                    for (int j = 0; j < straitRect.Height; j++)
                    {
                        ...
                        // If there is less than 4 Brinerack nearby, replace any Barosand with Brinerack 
                        if (sur3 < 4)
                        {
