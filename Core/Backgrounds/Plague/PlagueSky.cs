...
namespace CalRemix.Core.Backgrounds.Plague
{
    public class PlagueSky : CustomSky
    {
		...
		{
            SkyTexture = ModContent.Request<Texture2D>("CalRemix/Core/Backgrounds/Plague/PlagueSky").Value;
            DesertSkyTexture = ModContent.Request<Texture2D>("CalRemix/Core/Backgrounds/Plague/PlagueDesertSky").Value;

            _starTextures = new Texture2D[2];
			for (int i = 0; i < _starTextures.Length; i++)
			{
				_starTextures[i] = ModContent.Request<Texture2D>("CalRemix/Core/Backgrounds/Plague/PlagueStar" + i).Value;
			}
        }
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            if (maxDepth >= 3.40282347E+38f && minDepth < 3.40282347E+38f)
            {
                if (Main.player[Main.myPlayer].GetModPlayer<CalRemixPlayer>().ZonePlague)
                {
                    ...
                    spriteBatch.Draw(ModContent.Request<Texture2D>("CalRemix/Core/Backgrounds/Plague/PlagueSky").Value, new Rectangle(0, 0, Main.screenWidth, (int)height), Color.White * Intensity);
                }
