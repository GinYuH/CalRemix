...
namespace CalRemix.Subworlds
{
    public class ExosphereSky : CustomSky
    {
        ...
        public static bool CanSkyBeActive
        {
            get
            {
                return SubworldSystem.Current == ModContent.GetInstance<ExosphereSubworld>();
            }
        }
        ...
        public static void CreateLightningBolt(int count = 1, bool playSound = false)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                Lightning lightning = new Lightning()
                {
                    ...
                    Position = new Vector2(Main.LocalPlayer.Center.X + Main.rand.NextFloatDirection() * 5000f, Main.LocalPlayer.Center.Y - Main.rand.NextFloat(4850f))
                };
                (SkyManager.Instance["CalRemix:Exosphere"] as ExosphereSky).LightningBolts.Add(lightning);
            }

            // Make the sky flash if enough lightning bolts are created.
            if (count >= 10)
            {
                (SkyManager.Instance["CalRemix:Exosphere"] as ExosphereSky).LightningIntensity = 1f;
                playSound = true;
            }
