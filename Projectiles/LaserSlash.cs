            ...
            foreach (Player player in Main.player)
            {
                if (player.getRect().Intersects(Projectile.getRect()) && player.immuneTime <= 0)
                {
                    if (CalamityWorld.revenge)
                        player.Hurt(PlayerDeathReason.ByProjectile(Projectile.owner, Projectile.whoAmI), (int)(player.statLife / 10f), (player.Center.X > Projectile.Center.X) ? 1 : -1, dodgeable: false, armorPenetration: 10000);
                    else if (Main.expertMode)
                        player.Hurt(PlayerDeathReason.ByProjectile(Projectile.owner, Projectile.whoAmI), (int)(player.statLife / 20f), (player.Center.X > Projectile.Center.X) ? 1 : -1, dodgeable: false, armorPenetration: 10000);
                    else
                        player.Hurt(PlayerDeathReason.ByProjectile(Projectile.owner, Projectile.whoAmI), (int)(player.statLife / 40f), (player.Center.X > Projectile.Center.X) ? 1 : -1, dodgeable: false, armorPenetration: 10000);
                }
            }
