...
{
	public class PlagueWaterDroplet : ModGore
	{
		...
		public override bool Update(Gore gore)
		{
			...
			if (gore.frame <= 4)
			{
				...
				if ((int)gore.frameCounter >= frameDuration)
				{
					...
					if (gore.frame == 5)
					{
						int droplet = Gore.NewGore(new EntitySource_Misc("0"), gore.position, gore.velocity, gore.type, 1f);
			...
			if (gore.velocity != oldVelocity)
			{
				if (gore.frame < 10)
				{
					gore.frame = 10;
					gore.frameCounter = 0;
                    SoundEngine.PlaySound(Terraria.ID.SoundID.Drip, gore.position);
                }
			}
