...
namespace CalRemix.Tiles.Plates
{
    public class CurseplateTile : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            if (!Main.dedServ)
            {
                PulseTexture = Request<Texture2D>("CalRemix/Tiles/Plates/CurseplateTilePulse", (AssetRequestMode)1).Value;
                PulseColors = (Color[])(object)new Color[PulseTexture.Width];
                Main.QueueMainThreadAction(delegate
                {
                    PulseTexture.GetData<Color>(PulseColors);
                });
                GlowTexture = Request<Texture2D>("CalRemix/Tiles/Plates/CurseplateTileGlow", (AssetRequestMode)1).Value;
            }
